...
output_dir = "output/"
golden_data = "output/es.trial.txt"
...
for doc in golden_mentions:
    if "/" in doc:
        ...
        if doc_name in predict_mentions and len(predict_mentions[doc_name]) > 0:
            p_muc, r_muc, f1_muc = compute_muc(golden_mentions[doc], predict_mentions[doc_name])
            p_muc_final += p_muc
            r_muc_final += r_muc
            f1_muc_final += f1_muc

            p_b3, r_b3, f1_b3 = compute_bcubed(golden_mentions[doc], predict_mentions[doc_name])
            p_b3_final += p_b3
            r_b3_final += r_b3
            f1_b3_final += f1_b3

            p_ceaf, r_ceaf, f1_ceaf = compute_ceaf(golden_mentions[doc], predict_mentions[doc_name])
            p_ceaf_final += p_ceaf
            r_ceaf_final += r_ceaf
            f1_ceaf_final += f1_ceaf

            precision, recall, f1 = compute_mentions_metrics(
                golden_mentions[doc], predict_mentions[doc_name])
